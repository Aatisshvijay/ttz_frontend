...
const AvatarTemplesPage = ({ isDarkMode }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const response = await api.temples.getAvatarTemples(
          godName,
          categoryName
        );
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center py-16">
        ...
        >
          Loading temples for {categoryName}...
        </div>
      </div>
    );
  if (error)
  ...
  if (temples.length === 0)
    return (
      <div className="text-center py-12">
        ...
        >
          No temples were found for "{categoryName}" under "{godName}".
        </p>
        <button
          onClick={() => navigate("/")}
          className={`px-6 py-3 rounded-full font-semibold transition-colors duration-300 ${
  ...
  return (
    <div className="py-8">
      <div className="text-center mb-12">
        ...
        >
          Explore the sacred temples dedicated to {categoryName} (
